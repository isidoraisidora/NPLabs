...
class Timestamp<T> implements Comparable<Timestamp<?>>{
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof Timestamp<?> other)) return false;
...
class Scheduler<T>{
    ...
    public Timestamp<T> next(){
        ...
        Timestamp<T> next = null;
        for(Timestamp<T> t: elements){
            if(t.getTime().isAfter(now)){
                if(next == null || t.getTime().isBefore(next.getTime())){
                    next = t;
                }
            }
        }
    ...
    public Timestamp<T> last(){
        ...
        for(Timestamp<T> t : elements){
            if(t.getTime().isBefore(now)){
                if(last == null || t.getTime().isAfter(last.getTime())){
                    last = t;
                }
            }
        }
    ...
    public List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end){
        List<Timestamp<T>> newList = new ArrayList<>();
        for(Timestamp<T> t : elements){
